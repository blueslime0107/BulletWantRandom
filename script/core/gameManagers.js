...
export class ThreeManager {
  constructor() {
    ...
    this.particleRate     = null
    this.particleRotation = null
    this.particleRotSpeed = null
    this.particleTime     = 0
    this.particleCfg      = null
  }
  ...
  initBG(bg) {
    ...
    three.fog.near = bg.fogSetting.near ?? 300
    three.fog.far = bg.fogSetting.far ?? 500
    // 세그먼트 데이터 맵 생성
    ...
    if (bg.particles) this._initParticles(bg.particles);
  }

  initSky(bg) {
    if (this.sky) {
      three.scene.remove(this.sky);
    ...
    const skyGeo = new THREE.CylinderGeometry(
      bg.skyRadius, bg.skyRadius,
      bg.skyHeight, 48, 1, true
    );
    ...
    this.sky.position.set(0, bg.skyY, 0);
  ...
  _initParticles(cfg) {
    ...
    for (let i = 0; i < count; i++) {
      pos[i * 3]     = getRandom(cfg.spawnX[0], cfg.spawnX[1]);
      pos[i * 3 + 1] = getRandom(cfg.spawnY[0], cfg.spawnY[1]);
      pos[i * 3 + 2] = getRandom(cfg.spawnZ[0], cfg.spawnZ[1]);
      phase[i]  = Math.random() * Math.PI * 2;
      rate[i]   = Math.random();
      rot[i]    = Math.random() * Math.PI * 2;
      rotSpd[i] = getRandom(cfg.rotationSpeed[0], cfg.rotationSpeed[1]);
    }
    const geo = new THREE.BufferGeometry();
    geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
    geo.setAttribute('rotation', new THREE.BufferAttribute(rot, 1));
    const mat = new THREE.ShaderMaterial({
      uniforms: {
        uMap:  { value: ThreeTextures[cfg.texture] },
      ...
      vertexShader: `
        ...
        void main() {
          ...
          gl_PointSize = uSize * (500.0 / -mvPosition.z);
    ...
    this.particleRotation = rot;
    this.particleRotSpeed = rotSpd;
    this.particleTime     = 0;
    this.particleCfg      = cfg;
  ...
  update() {
    ...
    if (this.particles) {
      ...
      const rot   = this.particleRotation;
      const rotSpd= this.particleRotSpeed;
      this.particleTime += 0.016;
      this.particles.position.x = three.camera.position.x;
      const pos   = this.particles.geometry.attributes.position.array;
      const count = pos.length / 3;
      for (let i = 0; i < count; i++) {
        const ph = phase[i];
        const r  = rate[i];
        const fallSpeed    = cfg.fallSpeed[0]    + (cfg.fallSpeed[1]    - cfg.fallSpeed[0])    * r;
        const forwardSpeed = cfg.forwardSpeed[0] + (cfg.forwardSpeed[1] - cfg.forwardSpeed[0]) * r;
        pos[i * 3]     += Math.sin(this.particleTime + ph) * cfg.swayAmp;
        pos[i * 3 + 1] -= fallSpeed;
        pos[i * 3 + 2] += forwardSpeed;
        rot[i] += rotSpd[i];
        if (pos[i * 3 + 1] < cfg.recycleY) {
          pos[i * 3]     = getRandom(cfg.spawnX[0], cfg.spawnX[1]);
          pos[i * 3 + 1] = getRandom(cfg.respawnY[0], cfg.respawnY[1]);
          pos[i * 3 + 2] = getRandom(cfg.spawnZ[0], cfg.spawnZ[1]);
        }
        if (pos[i * 3 + 2] > cfg.recycleZ) {
          pos[i * 3 + 2] = getRandom(cfg.spawnZ[0], cfg.spawnZ[0] + (cfg.spawnZ[1] - cfg.spawnZ[0]) * 0.4);
        }
      }
...
export class AudioManager {
  ...
  async loadAudios(bgmSources = {}, sfxSources = {}, onProgress) {
    console.log("Loading audios...", { bgmSources, sfxSources });
    Object.assign(this.bgmSourcesMap, bgmSources);
    Object.assign(this.sfxSourcesMap, sfxSources);

    const entries = [
      ...Object.entries(bgmSources).map(([k, url]) => ({ type: "bgm", k, url:`resources/audio/${url}` })),
      ...Object.entries(sfxSources).map(([k, url]) => ({ type: "sfx", k, url:`resources/audio/${url}` })),
    ];
  ...
  setBGMVolume(v) {
    this.bgmVolume = Math.max(0, Math.min(100, v)) / 100;
    if (this.bgmGain) this.bgmGain.gain.value = this.bgmVolume;
  }
  setSFXVolume(v) {
    this.sfxVolume = Math.max(0, Math.min(100, v)) / 100;
...
export class OptionManager {
  ...
  init() {;
    ...
    this.applyOption({ ...gameData.defaultOption, ...loadedOption });
    Save.saveOption(this.option);
  }

  applyOption(option) {
    const normalized = { ...gameData.defaultOption, ...option };
  ...
  cycleLanguage(step = 1) {
    const total = Data.languages.length || 1;
    const current = this.option?.language ?? 1;
    const next = ((current - 1 + step) % total + total) % total + 1;
    this.setOption({ language: next });
    return next;
  }
